...
describe('BrowserInstance', () => {
  ...
  beforeEach(() => {
    ...
    instance = new BrowserInstance('test-id', mockMCP);
  });
  ...
  describe('constructor', () => {
    it('应该正确初始化浏览器实例', () => {
      ...
      expect(instance.status).toBe(BrowserStatus.UNINITIALIZED);
    });
  });
  ...
  describe('launch', () => {
    it('应该成功启动浏览器', async () => {
      ...
      expect(instance.status).toBe(BrowserStatus.IDLE);
    ...
    it('启动失败时应该抛出异常并设置状态为ERROR', async () => {
      ...
      expect(instance.status).toBe(BrowserStatus.ERROR);
    });
  ...
  describe('setCookie', () => {
    it('应该成功设置Cookie', async () => {
      ...
      expect(mockMCP.evaluate).toHaveBeenCalledWith(
  ...
  describe('navigate', () => {
    it('应该成功导航到URL', async () => {
      ...
      expect(instance.status).toBe(BrowserStatus.BUSY);
    });

    it('导航超时应该抛出异常', async () => {
      mockMCP.navigate.mockImplementation(
        () =>
  ...
  describe('isHealthy', () => {
    ...
    it('UNINITIALIZED状态应该返回false', () => {
      instance.status = BrowserStatus.UNINITIALIZED;
      expect(instance.isHealthy()).toBe(false);
    });
  });
